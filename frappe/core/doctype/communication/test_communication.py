...
class TestCommunication(unittest.TestCase):
	...
	def test_email(self):
		valid_email_list = ["Full Name <full@ektai.mail>",
		'"Full Name with quotes and <weird@chars.com>" <weird@ektai.mail>',
	...
	def test_contacts_attached(self):
		...
		contact_sender.add_email("comm_sender@ektai.mail")
		...
		contact_recipient.add_email("comm_recipient@ektai.mail")
		...
		contact_cc.add_email("comm_cc@ektai.mail")
		...
		comm = frappe.get_doc({
			...
			"sender": "comm_sender@ektai.mail",
			"recipients": "comm_recipient@ektai.mail",
			"cc": "comm_cc@ektai.mail"
		}).insert(ignore_permissions=True)
	...
	def test_link_in_email(self):
		...
		comm = frappe.get_doc({
			...
			"sender": "comm_sender@ektai.mail",
			"recipients": "comm_recipient+{0}+{1}@ektai.mail".format(quote("Note"), quote(note.name)),
		}).insert(ignore_permissions=True)
...
def create_email_account():
	...
	email_account = frappe.get_doc({
		...
		"enable_outgoing": 1,
		"smtp_server": "test.ektai.mail",
		"email_id": "test_comm@ektai.mail",
		"password": "password",
		"add_signature": 1,
		"signature": "\nBest Wishes\nTest Signature",
		"enable_auto_reply": 1,
		...
		"send_notification_to": "test_comm@ektai.mail",
		"pop3_server": "pop.test.ektai.mail",
		"no_remaining":"0",
