...
class TestUserPermission(unittest.TestCase):
	def setUp(self):
		frappe.db.sql("DELETE FROM `tabUser Permission` WHERE `user`='test_bulk_creation_update@ektai.mail'")

	def test_default_user_permission_validation(self):
		user = create_user('test_default_permission@ektai.mail')
		param = get_params(user, 'User', user.name, is_default=1)
		add_user_permissions(param)
		#create a duplicate entry with default
		perm_user = create_user('test_user_perm@ektai.mail')
	...
	def test_apply_to_all(self):
		...
		user = create_user('test_bulk_creation_update@ektai.mail')
