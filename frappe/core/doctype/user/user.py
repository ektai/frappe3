# Copyright (c) 2019, Ektai SAS and Contributors
...
class User(Document):
	...
	def check_demo(self):
		if frappe.session.user == 'demo@erpnext.com':
			frappe.throw(_('Cannot change user details in demo. Please signup for a new account at https://ektai.io'), title=_('Not Allowed'))
		...
		try:
			if self.flags.in_insert:
				if self.name not in STANDARD_USERS:
					...
					if not self.flags.no_welcome_mail and self.send_welcome_email and not self.flags.email_sent:
