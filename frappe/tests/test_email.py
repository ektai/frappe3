...
class TestEmail(unittest.TestCase):
	...
	def test_email_queue(self, send_after=None):
		frappe.sendmail(recipients=['test@ektai.mail', 'test1@ektai.mail'],
						sender="admin@ektai.mail",
		...
		self.assertTrue('test@ektai.mail' in queue_recipients)
		self.assertTrue('test1@ektai.mail' in queue_recipients)
	...
	def test_cc_header(self):
		...
		frappe.sendmail(recipients=['test@ektai.mail'],
						cc=['test1@ektai.mail'],
						sender="admin@ektai.mail",
						reference_doctype='User', reference_name="Administrator",
		...
		self.assertTrue('To: test@ektai.mail' in message)
		self.assertTrue('CC: test1@ektai.mail' in message)
	...
	def test_cc_footer(self):
		...
		self.assertTrue('This email was sent to test@ektai.mail and copied to test1@ektai.mail' in frappe.safe_decode(
	...
	def test_unsubscribe(self):
		...
		unsubscribe(doctype="User", name="Administrator", email="test@ektai.mail")
		...
		send(recipients=['test@ektai.mail', 'test1@ektai.mail'],
			 sender="admin@ektai.mail",
			 reference_doctype='User', reference_name="Administrator",
		...
		queue_recipients = [r.recipient for r in frappe.db.sql("""select recipient from `tabEmail Queue Recipient`
			where status='Not Sent'""", as_dict=1)]
		self.assertFalse('test@ektai.mail' in queue_recipients)
		self.assertTrue('test1@ektai.mail' in queue_recipients)
