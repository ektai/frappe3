...
class TestAutoAssign(unittest.TestCase):
	...
	def test_load_balancing(self):
		...
		for user in ('test@ektai.mail', 'test1@ektai.mail', 'test2@ektai.mail'):
		...
		for d in frappe.get_all('ToDo', dict(reference_type = 'Note', owner = 'test@ektai.mail'), limit=5):
	...
	def check_assignment_rule_scheduling(self):
		...
		self.assertIn(frappe.db.get_value('ToDo', dict(
			...
			reference_name = note.name,
			status = 'Open'
		), 'owner'), ['test@ektai.mail', 'test1@ektai.mail', 'test2@ektai.mail'])
...
def get_assignment_rule(days, assign=None):
	...
	assignment_rule = frappe.get_doc(dict(
		...
		users = [
			dict(user = 'test@ektai.mail'),
			dict(user = 'test1@ektai.mail'),
			dict(user = 'test2@ektai.mail'),
		]
	)).insert()
