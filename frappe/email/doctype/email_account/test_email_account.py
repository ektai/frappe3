...
class TestEmailAccount(unittest.TestCase):
	...
	def test_incoming(self):
		cleanup("test_sender@ektai.mail")
		...
		comm = frappe.get_doc("Communication", {"sender": "test_sender@ektai.mail"})
		self.assertTrue("test_receiver@ektai.mail" in comm.recipients)
	...
	def test_incoming_attached_email_from_outlook_plain_text_only(self):
		...
		comm = frappe.get_doc("Communication", {"sender": "test_sender@ektai.mail"})
		self.assertTrue("From: \"Microsoft Outlook\" &lt;test_sender@ektai.mail&gt;" in comm.content)
	...
	def test_outgoing(self):
		make(subject = "test-mail-000", content="test mail 000", recipients="test_receiver@ektai.mail",
			send_email=True, sender="test_sender@ektai.mail")
	...
	def test_sendmail(self):
		frappe.sendmail(sender="test_sender@ektai.mail", recipients="test_recipient@ektai.mail",
	...
	def test_print_format(self):
		make(sender="test_sender@ektai.mail", recipients="test_recipient@ektai.mail",
	...
	def test_threading(self):
		cleanup(["in", ['test_sender@ektai.mail', 'test@ektai.mail']])

		# send
		sent_name = make(subject = "Test", content="test content",
			recipients="test_receiver@ektai.mail", sender="test@ektai.mail",doctype="ToDo",name=frappe.get_last_doc("ToDo").name,
	...
	def test_threading_by_subject(self):
		...
		comm_list = frappe.get_all("Communication", filters={"sender":"test_sender@ektai.mail"},
	...
	def test_threading_by_message_id(self):
		...
		frappe.sendmail(recipients='test@ektai.mail', subject='test message for threading',
